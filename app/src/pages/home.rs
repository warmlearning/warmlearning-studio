...
// 不點名任何醫療診斷或治療狀態（spec.md 4.1⑤ 隱私處理備註）。標題句為 v15 更新，
// 敘事拆成 2–3 小段落（v15 版面變更），文字內容沿用原文不變，重點句不變。
const STUDENT_RESULTS: [StudentResult; 3] = [
    StudentResult {
        image: "/img/result-1.jpg",
        title: "從「英文跟不上」到重新願意學",
        narrative: &[
            "原本專注力較不集中、對英文沒有信心的孩子，透過重新建立基礎與學習方法，逐漸找到與英文學習的連結點，也開始相信自己做得到。",
        ],
    ...
    StudentResult {
        ...
        title: "從被推著走，到自己往前走",
        narrative: &[
            "原本需要家長不斷提醒、缺乏自主學習習慣的孩子，透過陪跑與學習方法建立，慢慢從「被要求學」轉變成「知道自己該怎麼學」。",
            "每日練習總結、逐步修正，誠實了解自己的狀況與清晰目標，調整出最適合自己的讀書節奏，運用天賦、擅長處來幫上學習大忙。",
            "不再帶著罪惡感休息，能更有效善用時間，玩樂休息取得平衡，相信自己是優秀的。",
        ],
    ...
    StudentResult {
        ...
        narrative: &[
            "家長願意與老師合作，不只關注孩子的成績，也願意一起調整陪伴方式，最後讓親子關係與孩子的學習狀態都逐漸變得更穩定。",
        ],
        ...
        <section class="bg-gradient-to-b from-mist-blue to-white">
            <div class="mx-auto flex max-w-7xl flex-col items-center gap-10 px-6 py-16 lg:flex-row lg:py-24">
                <div class="flex flex-1 flex-col items-start gap-5 text-left">
                    <p class="font-handwriting text-xl text-sky-blue">"♡ 知足上進，溫暖而堅定"</p>
                    <h1 class="text-5xl font-bold leading-[1.3] text-brand-blue lg:text-6xl">
                        "從英文開始，" <br/> "陪孩子建立走得長久的學習力。"
                    </h1>
                    <p class="text-lg leading-[1.7] text-slate-gray">
                ...
                <div class="relative flex-1">
                    ...
                    <div class="absolute bottom-4 right-4 rounded-xl bg-brand-blue/90 px-4 py-3 text-white shadow-lg sm:right-8">
                        <p class="text-base font-medium">"創辦人 Penny，英文教學經驗 7 年"</p>
                    </div>
                </div>
            </div>
        </section>
    ...
    view! {
        <section class="bg-white">
            <div class="mx-auto max-w-7xl px-6 py-16 lg:py-24">
                <h2 class="flex items-center justify-center gap-3 text-center text-4xl font-bold text-brand-blue">
                            ...
                            view! {
                                <Reveal delay_ms=(i as u32) * 110>
                                    <div class="flex flex-col items-center gap-3 text-center">
                                        ...
                                        <h3 class="text-2xl font-bold text-ink">{card.title}</h3>
                                        <p class="text-base leading-[1.7] text-slate-gray">{card.body}</p>
                                        <a href="/about" class="text-base font-medium text-brand-blue hover:underline">
                                            "了解更多"
                                        </a>
                                    </div>
                                </Reveal>
...
fn CoursesSection() -> impl IntoView {
    view! {
        <section class="bg-mist-blue">
            <div class="mx-auto max-w-7xl px-6 py-16 lg:py-24">
                <h2 class="text-center text-4xl font-bold text-brand-blue">"每個階段，都有適合的學習道路"</h2>
                <p class="mx-auto mt-4 max-w-3xl text-center text-base leading-[1.7] text-slate-gray">
                                ...
                                <Reveal delay_ms=(i as u32) * 110>
                                    <Card class="overflow-hidden flex flex-col".to_string()>
                                        ...
                                        <div class="flex flex-1 flex-col gap-2 p-5">
                                            <h3 class="text-2xl font-bold text-ink">{course.title}</h3>
                                            <p class="flex-1 text-base leading-[1.7] text-slate-gray">{course.intro}</p>
                                            <a
                                                href=format!("/courses{}", course.anchor)
                                                class="group inline-flex items-center gap-1 text-base font-medium text-brand-blue"
                                            >
                                                "了解更多"
...
fn LearningFlowSection() -> impl IntoView {
    view! {
        <section class="bg-white">
            <div class="mx-auto max-w-7xl px-6 py-16 lg:py-24">
                <h2 class="text-center text-4xl font-bold text-brand-blue">"從這裡開始你的學習旅程"</h2>
                <p class="mx-auto mt-4 max-w-3xl text-center text-base leading-[1.7] text-slate-gray">
                            ...
                            view! {
                                <Reveal delay_ms=(i as u32) * 110>
                                    <div class="flex flex-col items-center gap-2 text-center">
                                        <h3 class="text-xl font-bold text-brand-blue">{step.title}</h3>
                                        <p class="text-base leading-[1.7] text-slate-gray">{step.body}</p>
                                    </div>
                                </Reveal>
                            }
                        })
...
fn ResultsSection() -> impl IntoView {
    view! {
        <section class="bg-mist-blue">
            <div class="mx-auto max-w-7xl px-6 py-16 lg:py-24">
                <h2 class="text-center text-4xl font-bold text-brand-blue">"他們的學習故事"</h2>
                ...
                <div class="mt-10 grid grid-cols-1 gap-8 lg:grid-cols-3">
                    {STUDENT_RESULTS
                        ...
                        .map(|(i, result)| {
                            view! {
                                <Reveal delay_ms=(i as u32) * 110>
                                    // v15 版面改回大圖：照片維持原始比例、不裁切，置於卡片最上方，
                                    // 標題句在圖片下方用較大字級／品牌色呈現（spec.md 4.1⑤）
                                    <Card class="flex h-full flex-col gap-4 overflow-hidden p-0".to_string()>
                                        <img src=result.image alt=result.title class="w-full"/>
                                        <div class="flex flex-1 flex-col gap-3 p-6">
                                            <h3 class="text-2xl font-bold leading-snug text-brand-blue">
                                                {result.title}
                                            </h3>
                                            ...
                                            <p class="text-base font-medium text-brand-blue">{result.highlight}</p>
                                        </div>
                                    </Card>
                                </Reveal>
                            }
        ...
        <section class="bg-white">
            <Reveal class="mx-auto flex max-w-7xl flex-col items-center gap-10 px-6 py-16 lg:flex-row lg:py-24".to_string()>
                <div class="flex-1 text-center lg:order-2 lg:text-left">
                    <blockquote class="text-2xl font-medium leading-[1.7] text-ink">
                        "Hi，我是 Penny。我相信，每個孩子都能找到適合自己的學習方式。我們教的不只是英文，而是一輩子的學習能力。"
                    </blockquote>
                    <p class="mt-4 text-base text-slate-gray">
        ...
        <section class="bg-mist-blue">
            <Reveal class="mx-auto flex max-w-7xl flex-col items-center gap-8 px-6 py-16 lg:flex-row lg:justify-between lg:py-24".to_string()>
                <div class="flex items-center gap-4 text-center lg:text-left">
                    ...
                    <p class="max-w-xl text-xl leading-[1.7] text-ink">
